...
let setOutliers = function (d: BITAData) {
...
let Gauge = function (ww: number, hh: number, Data: BITAData) {
    var thicknessOfBottomLabel = 25, rimForClockNumbers = 20, durationTime = 3000, reading2angle = function (k: number) {
        var d = 90 + k * 360 / 100;
        d = d % 360;
        return d;
    }, margin = {
        ...
        bottom: .2,
        left: .5
    }, tooltip = d3.select("body").append("g").attr("class", "toolTip"), angle = d3.scaleLinear<number>().domain([0, 360]).range([0, 2 * Math.PI]), ticks = [], ticks1 = [], i, width = ww - margin.left - margin.right, height = hh - margin.top - margin.bottom, rad1 = (height - thicknessOfBottomLabel) / 2, rad2 = rad1 - rimForClockNumbers, svg = d3.select("body").append("svg").attr("x", margin.left).attr("y", margin.top).attr("width", width).attr("height", height).append("g"),
        arcfunc = d3.arc().innerRadius(rad2).outerRadius(rad1).cornerRadius(5)
        , def = svg.append('defs'), gradient = def.append('linearGradient'/*or 'radialGradient' */
        ).attr('id', 'grad').attr('x1', '0%').attr('y1', '0%').attr('x2', '100%').attr('y2', '100%');
    ...
    let centx = ww / 2, centy = (hh - thicknessOfBottomLabel * 0.75) * 0.5
        , outerbox = svg.append('rect').attr('class', 'outerb').attr('width', width).attr('height', height).attr('x', margin.left).attr('y', margin.top)
        , circled = svg.append('circle').attr('transform', `translate(${centx},${centy})`).attr('r', `${rad2}px`).attr('class', 'clockface')
        , text = svg.selectAll('ci').data(ticks).enter().append('text').attr('transform', `translate(${centx},${centy})`).attr('class', 'clocktext').attr('x', function (k) {
        ...
        , tickl = svg.selectAll('line').data(ticks1).enter().append('path').attr('class', 'ticks').attr('transform', `translate(${centx},${centy})`).attr('d', function (k) {
        ...
        , arcs = svg.selectAll('arc').data([[10, 90], [Data.compliantLowerValue, Data.almostOutlierLowerValue], [Data.almostOutlierLowerValue, Data.almostOutlierUpperValue], [Data.almostOutlierUpperValue, Data.compliantUpperValue]])
            ...
            .style('fill', function (k, i) {
                ...
                let kh = [d3.interpolate(k[0] - 100, k[0]), d3.interpolate(k[0] - 100, k[1])];

                function tween(t: number) {
                    let d1 = angle(reading2angle(kh[0](t))) + Math.PI / 2;
                    let d2 = angle(reading2angle(kh[1](t))) + Math.PI / 2;
        ...
        z`;
                    ...
                    tooltip.style("opacity", 0);
                }), RISKcolourpass = '', RISKcolour = '',

        image = svg.append('defs')
            .append('pattern')
            .attr('id', 'me2')
            .attr('width', 1)
        ...
        , RISK = svg.selectAll('bot').data([Data.currentOutlier ? '&#x2713;' : '&#x2715;', Data.monitorFlagName, Data.proposedOutlier ? '&#x2713;' : '&#x2715;']).enter().append('text').attr('transform', `translate(${centx},${thicknessOfBottomLabel * 0.75 + 2 * rad1})`).attr('class', 'text').attr('x', function (d, i) {
            ...
            var pass = false;
            if (i == 0) { pass = Data.currentOutlier; }
            else if (i == 2) { pass = Data.proposedOutlier; }
            else { ; }
